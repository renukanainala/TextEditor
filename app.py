...
'''# Enhanced Text Editor with Exciting Features
...
# ------------------------------ TEXT EDITOR ------------------------------
text_editor_frame = tk.Frame(main_application)
text_editor_frame.pack(fill=tk.BOTH, expand=True)

line_numbers = tk.Text(text_editor_frame, width=4, padx=3, takefocus=0, border=0,
                       background='lightgray', state='disabled', wrap='none')
line_numbers.pack(side='left', fill='y')

text_editor = tk.Text(text_editor_frame, wrap='word', relief=tk.FLAT)
text_editor.pack(fill=tk.BOTH, expand=True)

scroll_bar = tk.Scrollbar(text_editor_frame, orient='vertical', command=text_editor.yview)
scroll_bar.pack(side=tk.RIGHT, fill=tk.Y)
text_editor.config(yscrollcommand=scroll_bar.set)
...
def update_line_numbers():
    ...
    for i in range(1, int(text_editor.index('end-1c').split('.')[0]) + 1):
...
text_editor.bind('<<Modified>>', changed)
text_editor.bind('<KeyRelease>', lambda e: update_line_numbers())
